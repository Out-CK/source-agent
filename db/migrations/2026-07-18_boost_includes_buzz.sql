-- Post.boost_score now blends date-proximity with the buzz scorer's output,
-- so buzzing events rank higher in both the anonymous feed (boost desc) and
-- the taste-ranked deck (boost_score * 10 term in scoreFeed).
-- buzz_score is open-ended; >= 1.0 counts as buzzing, capped at 3 for scoring.
--
-- Replaces only the boost expression inside sync_nyc_discovery()'s Post upsert:
--   before: greatest(0.0, 1.0 - (e.event_date - current_date)::float / 60.0)
--   after:  0.7 * soonness + 0.3 * least(coalesce(e.buzz_score, 0), 3.0) / 3.0
--
-- Applied by re-creating the function from the canonical definition in
-- 2026-07-18_discovery_sync.sql with the expression swapped in; see that file
