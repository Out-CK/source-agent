...
-- Feeds the NYC Discovery app (nyc_discovery schema) from event_entry_database_v2.
-- sync_nyc_discovery() upserts every upcoming event into Entity/Occurrence/
-- Media/Post with deterministic ids (evt_/evto_/evtm_/evtp_ + event_entry_id),
-- and deactivates posts whose event date has passed. Rerun daily after parse.
...
begin
  ...
  select
    ...
    greatest(0.0, 1.0 - (e.event_date - current_date)::float / 60.0),
